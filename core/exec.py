...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        if self.__C.RESUME:
            ...
            if ('ckpt_' + self.__C.VERSION) in os.listdir(self.__C.CKPTS_PATH+'Model_' + str(self.__C.METHOD) + '/'):
                shutil.rmtree(self.__C.CKPTS_PATH + 'ckpt_' + self.__C.VERSION)

            os.mkdir(self.__C.CKPTS_PATH + 'Model_' + str(self.__C.METHOD) + '/' +'ckpt_' + self.__C.VERSION)
        ...
        # Training script
        for epoch in tqdm(range(start_epoch, self.__C.MAX_EPOCH)):
            ...
            print("Starting the enumeration of data loader...")
            # Iteration
            for step, (
                    img_feat_iter,
                    img_tag_iter,
                ...
                # print("1 ",img_feat_iter.shape)
                # print("2 ",img_tag_iter.shape)
                # print("3 ",ques_ix_iter.shape)
                # print("4 ",ans_iter.shape)
                # input()
                optim.zero_grad()

                img_feat_iter = img_feat_iter.cuda()
                img_tag_iter = img_tag_iter.cuda()
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    sub_img_tag_iter = \
                        img_tag_iter[accu_step * self.__C.SUB_BATCH_SIZE:
                                 (accu_step + 1) * self.__C.SUB_BATCH_SIZE]
                    print("sub_image_tag safe ",sub_img_tag_iter.shape)
                    sub_ques_ix_iter = \
